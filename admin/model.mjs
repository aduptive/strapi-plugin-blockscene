...
export function componentDefaults(schema, components, stack = []) {
  if (!schema || stack.includes(schema)) throw new Error('Invalid recursive component defaults')
  const row = {}
  for (const [name, attr] of Object.entries(schema.attributes || {})) {
    if (Object.hasOwn(attr, 'default')) {
      const value = structuredClone(attr.default)
      if (attr.type === 'password') row[name] = ''
      else if (attr.type === 'relation') row[name] = { connect: [], disconnect: [] }
      else if (value !== null || attr.type === 'boolean') row[name] = value
    } else if (attr.type === 'component' && attr.required) {
      const create = () => componentDefaults(components[attr.component], components, [...stack, schema])
      if (attr.repeatable) {
...
export function editableZones(schema, values, canEdit, disabled = false) {
  ...
  return Object.entries(schema?.attributes || {})
    .filter(([name, attr]) => attr.type === 'dynamiczone' && !attr.conditions && canEdit(name))
    .map(([name, attr]) => {
      const count = Array.isArray(values?.[name]) ? values[name].length : 0
      return { name, components: attr.components || [], max: attr.max, count, full: count >= (attr.max ?? Infinity) }
    })
}

export function canInsert(zone, uid, values, components, count = 1) {
  return Boolean(zone?.components.includes(uid) && components[uid] &&
    (Array.isArray(values?.[zone.name]) ? values[zone.name].length : 0) + count <= (zone.max ?? Infinity))
}
...
export function candidatesFor(uid, meta = {}, config = {}) {
  const version = config.previewVersion ? `?v=${encodeURIComponent(config.previewVersion)}` : ''
  const base = (config.previewBaseUrl || '/block-previews').replace(/\/$/, '')
  return [meta.manualImage, meta.image, `${base}/${encodeURIComponent(uid)}.webp${version}`].filter(Boolean)
}

// Strapi-managed attributes the editor never fills; CKEditor custom fields are rich text under the hood.
const SYSTEM_FIELDS = ['id', 'documentId', 'createdAt', 'updatedAt', 'publishedAt', 'createdBy', 'updatedBy', 'locale', 'localizations']
const fieldType = attr => attr?.type === 'customField' ? (String(attr.customField || '').includes('CKEditor') ? 'richtext' : 'customField') : attr?.type || 'unknown'
export const fieldsOf = schema => Object.entries(schema?.attributes || {}).filter(([name]) => !SYSTEM_FIELDS.includes(name)).map(([name, attr]) => ({ name, type: fieldType(attr) }))
export const categoryOf = (uid, schema, meta = {}) => meta.category || schema?.category || uid.split('.')[0]
// System-ish categories ("admin") sort after everything else; the rest alphabetical (section order and filter options).
export const byCategory = (a, b) => (a === 'admin' ? 1 : 0) - (b === 'admin' ? 1 : 0) || a.localeCompare(b)

export function entriesFor(zone, components, config, query, category = 'all') {
  const needle = query.trim().toLocaleLowerCase()
  return zone.components.flatMap(uid => {
    const schema = components[uid]
    if (!schema) return []
    const meta = config.components?.[uid] || {}
    const entry = { uid, label: meta.label || schema.info?.displayName || uid,
      description: meta.description || schema.info?.description || '',
      category: categoryOf(uid, schema, meta), fields: fieldsOf(schema),
      candidates: candidatesFor(uid, meta, config), template: meta.template || 'generic' }
    if (category !== 'all' && entry.category !== category) return []
    return `${entry.label} ${entry.description} ${entry.category} ${uid} ${meta.keywords || ''}`
      .toLocaleLowerCase().includes(needle) ? [entry] : []
  }).sort((a, b) => byCategory(a.category, b.category) || a.label.localeCompare(b.label))
}
...
export function categoriesFor(zone, components, config) {
  ...
  for (const uid of zone.components) { const schema = components[uid]; if (!schema) continue; const c = categoryOf(uid, schema, config.components?.[uid] || {}); counts.set(c, (counts.get(c) || 0) + 1) }
...
export function writeMemory(storage, key, value) {
  try { if (key && (value === 'open' || value === 'closed')) storage?.setItem(key, value) } catch { /* blocked storage keeps the default */ }
}
