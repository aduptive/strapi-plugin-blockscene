...
import { Modal as Dialog, Switch, Flex, Typography, Field, SingleSelect, SingleSelectOption, TextInput } from '@strapi/design-system'
import { useFetchClient, useRBAC, useStrapiApp, useAuth, useForm } from '@strapi/strapi/admin'
import { unstable_useContentManagerContext as useContext, useDocumentRBAC } from '@strapi/content-manager/strapi-admin'
...
function Modal({ open, onOpenChange, trigger, title, children }: any) {
  return <Dialog.Root open={open} onOpenChange={onOpenChange}><Dialog.Trigger>{trigger}</Dialog.Trigger>
    <Dialog.Content style={{ width: '80vw', maxWidth: '80vw' }}><Dialog.Header><Dialog.Title>{title}</Dialog.Title></Dialog.Header>
      <Dialog.Body>{children}</Dialog.Body></Dialog.Content></Dialog.Root>
}
function Panel() {
  ...
  const zones = editableZones(c.contentType, formValues ?? c.form?.values, (name: string) =>
    allowed.includes(name) && (creating || (rbac.canReadFields || []).includes(name)) &&
    fields.find((field: any) => field.name === name)?.disabled !== true,
    c.isLoading || c.form?.disabled || !c.form?.addFieldRow)
    .map(zone => ({ ...zone, label: fields.find((field: any) => field.name === zone.name)?.label || zone.name }))
  const add = (zone: any, uid: string) => {
    const values = formValues ?? c.form.values
    const close = catalog?.groups?.[uid]
    if (c.form.disabled || !canInsert(zone, uid, values, c.components, close ? 2 : 1) || (close && !zone.components.includes(close))) return false
    // Explicit indexes: two appends in one tick would both target the same stale length and land reversed.
    const at = Array.isArray(values?.[zone.name]) ? values[zone.name].length : 0
    c.form.addFieldRow(zone.name, { ...componentDefaults(c.components[uid], c.components), __component: uid }, at)
  ...
  return { title: t.gallery, content: <Flex direction="column" alignItems="stretch" gap={4}>
    <Gallery zones={zones} components={c.components} add={add} Modal={Modal} Toggle={ToggleField} get={get} editor={catalog.editor} docKey={docKey} contentType={c.model} userId={user?.id} />
    <PagePreview editor={catalog.editor} groups={catalog.groups || null} Toggle={ToggleField} />
  </Flex> }
}
const ToggleField = ({ name, label, value, onChange, disabled }: any) =>
  <Flex gap={3}><Switch name={name} aria-label={label} checked={Boolean(value)} disabled={disabled} onCheckedChange={onChange} /><Typography>{label}</Typography></Flex>
const TextField = ({ name, label, value, onChange, disabled, placeholder }: any) =>
  <Field.Root name={name}><Field.Label>{label}</Field.Label><TextInput name={name} value={value} disabled={disabled} placeholder={placeholder} onChange={(e: any) => onChange(e.target.value)} /></Field.Root>
const SelectField = ({ name, label, value, onChange, disabled, options }: any) =>
  <Field.Root name={name}><Field.Label>{label}</Field.Label>
    <SingleSelect value={value} disabled={disabled} onChange={(v: any) => onChange(String(v))}>
