...
function Card({ entry, palette, showFields, onSelect }: any) {
  ...
  return (
    <Tile
      ...
      onClick={onSelect}
      title={entry.uid}
      data-testid={`blockscene-${entry.uid}`}
    >
      <Thumb
...
function ZoneGallery({
  ...
  const entries = config
    ? entriesFor(zone, components, config, query, category)
    : [];
    ...
    >
      <Flex direction="column" alignItems="stretch" gap={3}>
        ...
        <Flex gap={3} alignItems="center" wrap="wrap">
          ...
          >
            {filtering
              ? t.f("countFiltered", { shown: entries.length, total })
              : t.f("countAll", { count: total })}
          </Typography>
          ...
          {categories.length > 1 && (
            <Box
              style={{ width: "14rem" }}
              data-testid={`block-category-${zone.name}`}
            >
              <SingleSelect
                aria-label={t.filterCategory}
                size="S"
                value={category}
                onChange={(value: any) => setCategory(String(value))}
              >
                <SingleSelectOption value="all">
                  {t.allCategories}
                </SingleSelectOption>
                {categories.map(([name, count]: any) => (
                  <SingleSelectOption
                    key={name}
                    value={name}
                  >{`${name} (${count})`}</SingleSelectOption>
                ))}
              </SingleSelect>
            </Box>
          )}
          ...
          <Flex gap={2} alignItems="center">
            ...
            <Slider
              ...
              $percent={
                ((columns - COLUMNS.min) / (COLUMNS.max - COLUMNS.min)) * 100
              }
        ...
        {error ? (
          ...
          <Flex
            ...
            alignItems="stretch"
            gap={4}
            style={{ overflowY: "auto", maxHeight: "60vh", padding: "4px" }}
          >
            {groupEntries(entries).map(
              ({ category: name, entries: items }: any) => (
                <Flex
                  key={name}
                  direction="column"
                  alignItems="stretch"
                ...
                >
                  <Typography
                    variant="sigma"
                    textColor="neutral600"
                    data-testid={`block-group-${name}`}
                  >{`${name} (${items.length})`}</Typography>
